...
Socket.prototype.connect = function(port, host, connectListener){
    ...
    function looked(err, ip, family) {
        ...
        var udpSocket = this._udpSocket = dgram.createSocket('udp' + family);

        function write (buf, doneListener) {
            udpSocket.send(buf, 0, buf.length, port, ip, doneListener);
        }
        ...
        function close () {
            ...
            self.removeEventListener('_write', write);
